...
##function for aggregation to SUPER_ROIs (mean) -> df new & all new ROI names
def aggregate_to_super_rois(
    results_df: pd.DataFrame, 
    original_roi_names: List[str], 
    core_map: dict,
    method: str,
    aggregate: bool = None
):
    # results_df : pd.DataFrame
    #     DataFrame mit region_X_z_score Spalten
    # original_roi_names : List[str]
    #     Liste der originalen ROI-Namen
    # core_map : dict
    #     Mapping von Super-ROI zu Liste von Core-ROI-Namen
    # method : str, default="mean"
    #     Aggregationsmethode: "mean", "median", "stouffer"
    # aggregate : bool, default=True
    #     Wenn False, wird keine Aggregation durchgeführt (gibt Original zurück)
  
    
    # NEUE OPTION: Aggregation überspringen
    if not aggregate:
        print("[INFO] Aggregation deaktiviert. Verwende Original-ROIs.")
        return results_df, original_roi_names
    
    # Ab hier: Dein ursprünglicher Code
    region_cols = [col for col in results_df.columns if col.startswith("region_")]
    
    if len(region_cols) != len(original_roi_names):
        print(f"[ERROR] Spaltenanzahl ({len(region_cols)}) stimmt nicht mit ROI-Namen ({len(original_roi_names)}) überein. Abbruch der Aggregation.")
        return results_df, original_roi_names

    # 1. Erstelle Mapping: {Region_Spaltenname : (Kern-ROI-Name, VolumeType)}
    col_to_parsed_name = {}
    for i, roi_name_full in enumerate(original_roi_names):
        parts = roi_name_full.split('_')
        ...
        col_to_parsed_name[region_cols[i]] = (core_name, volume_type)

    super_roi_zscores = pd.DataFrame(index=results_df.index)
    new_super_roi_names = []
    aggregation_groups = {}
    
    for super_roi_name, core_roi_list in core_map.items():
        for core_roi in core_roi_list:
            for col_name, (parsed_core_name, volume_type) in col_to_parsed_name.items():
                if parsed_core_name == core_roi:
                    key = (super_roi_name, volume_type)
                    if key not in aggregation_groups:
                        aggregation_groups[key] = []
                    aggregation_groups[key].append(col_name)

    # 2. Aggregation durchführen - HIER IST DER UNTERSCHIED
    aggregated_columns = {}
    for (super_roi_name, volume_type), col_list in aggregation_groups.items():
        
        new_col_name_raw = f"{super_roi_name}_{volume_type}"
        new_super_roi_names.append(new_col_name_raw)
        
        # NEUE OPTION: Verschiedene Aggregationsmethoden
        if method == "mean":
            aggregated_columns[new_col_name_raw] = results_df[col_list].mean(axis=1)
        elif method == "median":
            aggregated_columns[new_col_name_raw] = results_df[col_list].median(axis=1)
        elif method == "stouffer":
            z_values = results_df[col_list].values
            n_rois = len(col_list)
            combined_z = np.sum(z_values, axis=1) / np.sqrt(n_rois)
            aggregated_columns[new_col_name_raw] = combined_z
        else:
            raise ValueError(f"Unbekannte Methode: {method}. Verfügbar: 'mean', 'median', 'stouffer'")

    if not aggregated_columns:
        print("[WARNING] Keine Regionen konnten Super-ROIs zugeordnet werden. Weiter mit Original-ROIs.")
        return results_df, original_roi_names

    # 3. DataFrame aktualisieren (wie vorher)
    non_region_cols = [col for col in results_df.columns if not col.startswith("region_")]
    
    new_super_roi_df = pd.DataFrame(aggregated_columns)
    new_super_roi_df.columns = [f"region_{i}_z_score" for i in range(len(new_super_roi_names))]
    new_super_roi_df.index = results_df.index

    new_results_df = pd.concat([results_df[non_region_cols], new_super_roi_df], axis=1)
    
    print(f"[INFO] Aggregation mit '{method}' abgeschlossen: {len(original_roi_names)} → {len(new_super_roi_names)} Super-ROIs")
    
    return new_results_df, new_super_roi_names
